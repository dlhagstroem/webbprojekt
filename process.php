...
if($_POST) //Post Data received from product list page.
{
    ...
    foreach($_POST['item_name'] as $key=>$itmname)
    {
		...
		$res = $mysqli->query("SELECT id, name, description, price FROM products WHERE id='$product_code' LIMIT 1");
		$row = $res->fetch_object();
		
        $paypal_data .= '&L_PAYMENTREQUEST_0_NAME'.$key.'='.urlencode($row->name);
        ...
        $subtotal = ($row->price*$_POST['item_qty'][$key]);

        //total price
        $ItemTotalPrice = $ItemTotalPrice + $subtotal;
		
		//create items for session
		$paypal_product['items'][] = array('itm_name'=>$row->name,
											'itm_price'=>$row->price,
